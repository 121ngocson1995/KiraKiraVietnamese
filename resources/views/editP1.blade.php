...
<div class="container">
	...
	<div id="p1Div">
		...
		<div class="row" id="row{{$i}}" data-line="{{$i}}">
			<div class="col-xs-4">
				<input type="text" id="word{{$p1[$i]->wordNo}}" maxlength="10" name="word{{$p1[$i]->wordNo}}" value="{{$p1[$i]->word}}">
			</div>
			<div class="col-xs-4">
				<label for="audio{{$p1[$i]->wordNo}}">Audio</label>
				<input id="audio{{$p1[$i]->wordNo}}" name="audio{{$p1[$i]->wordNo}}" type="file" class="file undone audio" data-situ="{{$p1[$i]->wordNo}}" data-path-audio="{{$p1[$i]->audio}}" data-show-upload="false" data-show-caption="true" data-allowed-file-extensions='["mp3"]'>
			</div>
			<div class="col-xs-4">
